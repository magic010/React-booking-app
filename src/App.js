...
function App() {
  ...
  const updateBooksData = async (updatedBook, shelf) => {
    ...
    setBooksData(
      ...
      booksData.filter((book) => book.id !== updatedBook.id).concat(updatedBook) // then add it again with the new shelf value
    );
  };
    ...
    <div className="app">
      <Routes>
        <Route
          path="/search"
          element={
            <Search booksData={booksData} onUpdateShelfType={updateShelfType} />
          }
        />
        <Route
          exact
